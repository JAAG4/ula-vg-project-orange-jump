...
class TileInstance:
    ...
    def __init__(
        self, a: float, d: List[int], f: int, px: List[int], src: List[int], t: int
    ) -> None:
        self.a = a
        self.d = d
        self.f = f
...
def ldtk_from_file(path: str) -> LdtkJSON:
    with open(path, mode="r", encoding="utf-8") as ldtkf:
        return ldtk_json_from_dict(json.load(ldtkf))
